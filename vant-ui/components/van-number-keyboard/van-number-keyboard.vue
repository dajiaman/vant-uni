		...
		<view class="u-keyboard-grids">
			...
			<view class="u-keyboard-grids-item u-bg-gray" hover-class="u-hover-class" :hover-stay-time="100" @touchstart.stop="backspaceClick" @touchend="clearTimer">
				<view class="u-keyboard-back u-keyboard-grids-btn">
					<van-icon name="/static/images/icon_keyboard_delete.png" size="60rpx" :bold="true"></van-icon></view>
			</view>
		</view>
	</view>
</template>
		...
		numList() {
			...
			if (!this.dotEnabled && this.mode == 'number') {
				if (!this.random) {
					...
					return this.$van.randomArray([1, 2, 3, 4, 5, 6, 7, 8, 9, 0]);
				}
			} else if (this.dotEnabled && this.mode == 'number') {
				if (!this.random) {
					return [1, 2, 3, 4, 5, 6, 7, 8, 9, this.dot, 0];
				} else {
					return this.$van.randomArray([1, 2, 3, 4, 5, 6, 7, 8, 9, this.dot, 0]);
				}
			} else if (this.mode == 'card') {
				if (!this.random) {
					return [1, 2, 3, 4, 5, 6, 7, 8, 9, this.cardX, 0];
				} else {
					return this.$van.randomArray([1, 2, 3, 4, 5, 6, 7, 8, 9, this.cardX, 0]);
				}
			}
		},
