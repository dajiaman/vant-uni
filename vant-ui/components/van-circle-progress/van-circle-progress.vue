...
export default {
	...
	data() {
		return {
			...
			// #endif
			// #ifndef MP-WEIXIN
			elBgId: this.$van.guid(), // 非微信端的时候，需用动态的id，否则一个页面多个圆形进度条组件数据会混乱
			elId: this.$van.guid(),
			// #endif
	...
	computed: {
		// 有type主题时，优先起作用
		circleColor() {
			if (['success', 'error', 'info', 'primary', 'warning'].indexOf(this.type) >= 0) return this.$van.color[this.type];
	...
	methods: {
		...
		drawCircleByProgress(progress) {
			...
			if(this.inactive){
					ctx.setStrokeStyle('rgb(202, 202, 202)');
			}
