<template>
    <view class="van-cell van-field" :class="[
            disabled ? 'van-field--disabled' : '', 
            required ? 'van-cell--required' : '', error ? 'van-field--error' : '',
        ...
        <view class="van-cell__title van-field__label" v-if="label">
            <text>{{ label }}</text>
        </view>
        <view class="van-cell__value van-field__value">
            <view class="van-field__body">
                <textarea v-if="type == 'textarea'" class="van-field__control" :value="value" :placeholder="placeholder"
                    :disabled="disabled || readonly" :readonly="readonly ? 'readonly' : false" auto-height
                    :focus="focus" @focus="focusEvent" @blur="blur" @input="input" :maxlength="maxlength"
                    :class="[inputAlign ? 'van-field__control--' + inputAlign : '']" />

                <input :type="type" v-else class="van-field__control" :value="value" :password="password"
                    :disabled="disabled || readonly" :maxlength="maxlength" :readonly="readonly ? 'readonly' : false"
                    :placeholder="placeholder" :focus="focus" @focus="focusEvent" @blur="blur" @input="input"
                    @confirm="confirm" @linechange="linechange"
                    :class="[inputAlign ? 'van-field__control--' + inputAlign : '']" />

                <van-icon v-if="clearable && clearIconShow" class="van-field__clear" name="clear" size="38rpx"
