	...
	<view class="wrap">
		<view class="item-warp">
			<view class="item" v-for="(item, index) in list" :key="index">
				<van-lazyload threshold="-450" height="400" img-mode="aspectFill" border-radius="10" :image="item.src" :index="index" @statusChange="statusChange" @clickImg="clickImg"></van-lazyload>
			</view>
		</view>
		<van-loadmore :status="status" @loadmore="getData"></van-loadmore>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			getData() {
				...
				setTimeout(() => {
					for(let i = 0; i < 10; i++) {
						index = this.$van.random(0, this.data.length - 1);
						this.list.push({
